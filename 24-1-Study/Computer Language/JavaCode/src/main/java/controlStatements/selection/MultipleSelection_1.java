...
public class MultipleSelection_1 {
    public static void main(String[] argrs){
        ...
        System.out.println("Enter the integer grades in the range 0 to 100.");
        System.out.println("Enter -1 to finish entering grades.");

        while(true) { // 무한 루프로 변경
            int grade = scanner.nextInt();
            if(grade == -1) { // -1을 입력받으면 루프 종료
                break;
            }

            // 유효한 성적 범위(0~100) 내에서만 계산을 진행
        ...
        if(counter > 0){
            ...
            System.out.printf("Class average is %.2f%n", avg); // 평균 출력 시 avg 변수 포함
